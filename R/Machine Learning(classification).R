...
df_stroke<-data <- read.csv("stroke.csv", header=TRUE)
df_stroke
...
#Linear Regression
linear_model <- glm(stroke ~ ., data = train, family = binomial)
linear_predictions <- predict(linear_model, test, type = "response")
linear_predictions <- ifelse(linear_predictions > 0.5, 1, 0)
confusionMatrix(linear_predictions, test$stroke)
...
confusionMatrix(logistic_predictions, test$stroke)

#Support Vector Machine (SVM)
library(e1071)
svm_model <- svm(stroke ~ ., data = train, kernel = "linear", probability = TRUE)
svm_predictions <- predict(svm_model, test, probability = TRUE)
svm_predictions <- ifelse(attr(svm_predictions, "decision.values")[,1] > 0, 0, 1)
confusionMatrix(svm_predictions, test$stroke)
